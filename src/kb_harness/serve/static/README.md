...
`graph.html` は Sigma.js と Graphology を使う知識グラフビューアである。依存ライブラリは `vendor/` に、見た目の定義は `ds/` に同梱し、外部ランタイム、CDN、Claude Design の再エクスポートには依存しない。
...
- 型の色トークン（`--type-course` など）は上流の題材に固有なので使わない。型の色は `viewer.py` の `PALETTE` から型の定義順に割り当てる。先頭 4 色は上流の neon 4 色と同じ並びである
