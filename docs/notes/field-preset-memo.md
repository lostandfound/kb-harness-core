...
**当時に無かった論点: フィールドと値 Claim の分担。** ハーネスには型付きの値を置く容器がすでにある。`vocabulary.yml` の `properties` と値 Claim で、`year-expression` の正本はコア側にある。述語で「出典なしで断言できる深さまでが relation、それより細かい主張は Claim」と決めた物差しをそのまま値に当てると、フィールドは争いのない値（型検査なし、エンティティの `sources` が支える）、値 Claim は出典・確度・型を伴う値、という分担になる。この分担なら、フィールド層に型検査を足す理由はいっそう薄い。紀元前や 3 桁の年が要るのは歴史 KB で生没を Claim にするときで、そのときにコアの年表現を広げればよく、フィールドの都合でコアを触る必要はない。上の「見送り」はこの根拠で支持される。
