...
- ERROR: `domain` / `range` が型名のリストでない（スカラーは 1 要素として扱った上で違反として報告する）。`broader` の先が存在しない、自身か `related-to` を指す、循環する（循環は循環上の述語だけを 1 件で報告し、循環に至るだけの述語は責めない）。子の `domain` / `range` が直接の親の部分集合でない（親が無制約の側は任意。親に制約があり子が無制約なら違反）。`maps_to` が空でない文字列のリストでない。
