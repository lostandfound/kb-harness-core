...
CI は Python 3.12 で `python3 -m pytest` のみ。リンタは未導入。`tests/test_distribution_alignment.py` は兄弟ディレクトリ `../kb-ontology-core`（`pyproject.toml` が宣言するタグと同じ版）を前提とする。
...
- `kb_harness.ontology`（旧 `scripts/ontology_adapter.py`）が橋渡しで、責務は 2 つだけ：(a) `kb_ontology_core` の import 解決（pip 導入か、兄弟ディレクトリ `../kb-ontology-core/src` へのフォールバック）、(b) コアの `Diagnostic` をハーネスの構造化診断（`code` / `field` / `context`）へ翻訳。
