...
1. `python3 scripts/cinii_search.py "<検索語>" --count 5` を実行する。アプリケーション ID は `.env`（gitignore 済み、`CINII_APP_ID=...`）から自動で読まれる。未設定エラーが出たらユーザーに `.env` への設定を依頼する（ID の値をチャットやリポジトリに書かせない）。

2. 出力される references.yml 登録案から、対象エンティティの主張を実際に裏付けられそうな文献を選ぶ。タイトルだけで判断できない場合は URL を WebFetch して要旨を確認する。

3. 登録案の ID を規約に沿って編集する（著者ローマ字姓-年、例: `kadekaru-2017`。スクリプトの仮 ID は日本語著者名のままなので必ず直す）。`<content_root>/references.yml`（content_root は `kb-domain.yml` の `domain.content_root`）に追記する。フィールドは確認できた値のみ書く。特定系統内の資料と判断できる場合は optional key `lineage`（系統名の文字列。単位の例は `kb-domain.yml` の `domain.lineage_example`）を付与する（判断がつかない場合は省略）。
